...
namespace Verable.Client.DotNet
{
    public class VerableBeacon
    {
        ...
        public async Task<List<ServiceDefinition>> DiscoverOne(string serviceName)
        {
            var request = Pack(Constants.Command.DiscoverOne, serviceName);
            var response = await Send(request);
            var result = Unpack<List<ServiceDefinition>>(response.Response, true);
            return result;
        }

        public async Task<Dictionary<string, List<ServiceDefinition>>> DiscoverN(List<string> serviceNames)
        {
            var request = Pack(Constants.Command.DiscoverN, serviceNames, true);
            var response = await Send(request);
